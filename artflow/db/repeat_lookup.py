...
def _direct_generation_id(raw: str | int | None) -> int | None:
    value = str(raw or "").strip()
    if value.startswith("web:"):
        value = value[len("web:") :]
    if not value.startswith(_DB_ID_PREFIX):
        return None
    clean = value[len(_DB_ID_PREFIX) :]
    if not clean.isdigit():
        return None
    generation_id = int(clean)
...
async def get_repeat_task_by_any_id(session: AsyncSession, raw_task_id: str | int | None) -> Generation | None:
    # Callbacks generated by our UI namespace the local Generation.id as db_<id>.
    # Resolve that exact database identity before considering provider task ids,
    # eliminating collisions with providers that use numeric task identifiers.
    direct_generation_id = _direct_generation_id(raw_task_id)
    if direct_generation_id is not None:
        result = await session.execute(
            select(Generation)
            .where(Generation.id == direct_generation_id)
            .limit(1)
        )
    ...
    # Exact provider/local task id is the strongest match for legacy callbacks.
    result = await session.execute(
        select(Generation)
    ...
    # Numeric DB id remains supported for legacy numeric callbacks.
