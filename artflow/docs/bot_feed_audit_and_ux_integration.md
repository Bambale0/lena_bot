...
2. callback `gen:share:<id>` проверяет владельца результата и правила публикации;
3. `repo.share_to_feed` публикует только done-result, принадлежащий пользователю, с `result_url`;
4. для изображений производные от чужой ленты по-прежнему не публикуются через bot flow;
5. для видео пользователь может опубликовать собственный новый ролик даже если исходником был чужой пост из ленты;
6. при этом исходный чужой промпт остаётся скрытым: копирование и `gen:library:<id>` по-прежнему запрещены;
7. пользователь получает deep link на пост.
