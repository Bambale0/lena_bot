...
Документ фиксирует текущее устройство публичного сайта APIX и выводы после ревью интерфейса. Сайт не заменяет Telegram-бот и не трогает Telegram WebApp: это отдельный web-слой для домена `apix.chillcreative.ru`.
