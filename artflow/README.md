...
Зависшие задачи Suno проверяются сервером через KIE каждые 5 минут после первых 3 минут ожидания (интервал и минимальный возраст задаются `MUSIC_RECONCILE_INTERVAL_SECONDS` и `MUSIC_RECONCILE_MIN_AGE_SECONDS`). Подтверждённый отказ завершает генерацию с возвратом кредитов; успешный результат сохраняется из Suno record-info. Временная ошибка опроса оставляет задачу активной для следующей проверки. Ручной разовый запуск: `python scripts/reconcile_stuck_generations.py` в окружении приложения.
...
# Genjutsu renders outlive the shared 600s video poll budget
HIGGSFIELD_POLL_TIMEOUT_SECONDS=1800
HIGGSFIELD_STALE_TIMEOUT_SECONDS=2400
