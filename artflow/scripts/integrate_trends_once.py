...
def replace(path: str, old: str, new: str, *, count: int = 1) -> None:
    ...
    if actual < count:
        raise RuntimeError(
            f"Replacement marker not found in {path}: expected {count}, got {actual}\n{old[:300]}"
        )
...
''',
    ...
    replace(
        ...
        '        reply_markup=empty_feed_kb(top_day=top_day),\n',
    )
    replace(
        "artflow/bot/handlers/feed.py",
...
''',
    ...
    tests = ROOT / "artflow/tests/test_trends_surface.py"
    tests.write_text(
        '''from pathlib import Path
...
''',
    ...
    docs = ROOT / "artflow/docs/trends.md"
    docs.write_text(
        '''# Тренды APIX
