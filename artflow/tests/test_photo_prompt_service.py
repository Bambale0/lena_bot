...
@pytest.mark.asyncio
async def test_generate_prompt_from_photo_uses_openrouter_first(monkeypatch) -> None:
    calls: list[tuple[str, str]] = []

    async def fake_openrouter_gpt(model: str, image_data_url: str) -> str:
        calls.append((model, image_data_url))
        return "openrouter prompt"

    monkeypatch.setattr(photo_prompt_service.openrouter_client, "configured", lambda: True)
    monkeypatch.setattr(photo_prompt_service, "_call_openrouter_gpt", fake_openrouter_gpt)
    monkeypatch.setattr(
    ...
    assert result == "openrouter prompt"
    assert calls[0][0] == "openai/gpt-5.2"
    assert calls[0][1].startswith("data:image/jpeg;base64,")


@pytest.mark.asyncio
async def test_generate_prompt_from_photo_force_openrouter_blocks_fallbacks(monkeypatch) -> None:
    async def fail_openrouter_gpt(model: str, image_data_url: str) -> str:
    ...
    async def fail_comet(*_args, **_kwargs):
        raise AssertionError("CometAPI should not be called when OpenRouter is forced")

    monkeypatch.setattr(photo_prompt_service.openrouter_client, "configured", lambda: True)
    monkeypatch.setattr(photo_prompt_service.openrouter_client, "force_migrated_models", lambda: True)
    monkeypatch.setattr(photo_prompt_service, "_call_openrouter_gpt", fail_openrouter_gpt)
    monkeypatch.setattr(photo_prompt_service, "_call_kie_model", fail_kie)
    monkeypatch.setattr(photo_prompt_service, "_call_comet_gpt", fail_comet)
    monkeypatch.setattr(
        photo_prompt_service,
        "settings",
        SimpleNamespace(
            ...
            COMET_BASE_URL="https://api.cometapi.com",
            COMET_ASSISTANT_MODEL="gpt-5.4",
            COMET_ASSISTANT_FALLBACK="",
            KIE_AI_KEY="test-kie",
            KIE_PHOTO_PROMPT_MODEL="gpt-5-2",
            KIE_PHOTO_PROMPT_FALLBACK="",
        ),
    )

    with pytest.raises(RuntimeError, match="Photo prompt OpenRouter models are unavailable"):
        await photo_prompt_service.generate_prompt_from_photo(b"image", "image/jpeg")
