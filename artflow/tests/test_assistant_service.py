...
@pytest.mark.asyncio
async def test_generate_assistant_reply_uses_openrouter_first(monkeypatch) -> None:
    calls: list[tuple[str, list[dict[str, str]]]] = []

    async def fake_openrouter_chat(model: str, messages: list[dict[str, str]], *, system_prompt: str) -> str:
        calls.append((model, messages))
        return "OpenRouter OK"

    monkeypatch.setattr(assistant_service.openrouter_client, "configured", lambda: True)
    monkeypatch.setattr(assistant_service, "_call_openrouter_chat", fake_openrouter_chat)
    monkeypatch.setattr(
        assistant_service,
        "settings",
        SimpleNamespace(
    ...
    assert reply == "OpenRouter OK"
    assert calls[0][0] == "openai/gpt-5.4"


@pytest.mark.asyncio
async def test_generate_assistant_reply_force_openrouter_blocks_fallbacks(monkeypatch) -> None:
    async def fail_openrouter_chat(model: str, messages: list[dict[str, str]], *, system_prompt: str) -> str:
    ...
    monkeypatch.setattr(assistant_service.openrouter_client, "configured", lambda: True)
    monkeypatch.setattr(assistant_service.openrouter_client, "force_migrated_models", lambda: True)
    monkeypatch.setattr(assistant_service, "_call_openrouter_chat", fail_openrouter_chat)
    monkeypatch.setattr(assistant_service, "_call_kie_responses", fail_kie)
    monkeypatch.setattr(assistant_service, "_call_comet_chat", fail_comet)
    monkeypatch.setattr(
        assistant_service,
        "settings",
        SimpleNamespace(
    ...
    with pytest.raises(RuntimeError, match="Assistant OpenRouter models are unavailable"):
        await assistant_service.generate_assistant_reply([{"role": "user", "content": "Ping"}])
