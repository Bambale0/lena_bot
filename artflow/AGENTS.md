...
## Mandatory synchronized product surfaces

Любое пользовательское изменение продукта должно обновляться **синхронно во всех применимых контурах APIX**:

1. **Сайт / web**.
2. **Telegram Mini App**.
3. **Текстовый Telegram-бот**.

Нельзя обновить один контур и молча оставить другие со старым UX, текстом, кнопками, тарифами, платежными методами, параметрами модели, валидацией, доступностью функции или бизнес-логикой.

Для каждой задачи, затрагивающей пользовательское поведение, агент обязан:

1. До изменения определить, какие из трёх контуров затронуты.
2. Проверить существующую реализацию одной и той же функции во всех трёх контурах.
3. В рамках одной задачи и одного delivery cycle обновить все применимые контуры.
4. Обновить или добавить проверки для каждого затронутого контура.
5. Перед завершением проверить parity: одна и та же функция, правила, цены, названия, доступные параметры и ожидаемое поведение не должны расходиться между сайтом, Mini App и текстовым ботом.
6. В PR и финальном отчёте явно указать статус каждого контура: `site`, `mini_app`, `telegram_bot`.

Если изменение **технически неприменимо** к одному из контуров, это допустимое исключение только при явном объяснении в PR/отчёте. Нужно также проверить, что в этом контуре не осталось устаревшего или противоречащего поведения.

Если пользователь явно просит изменить только один контур, выполнить его прямое указание, но отдельно отметить возможный parity-разрыв. Во всех остальных случаях запрос на изменение пользовательской функции означает обязательную синхронизацию всех применимых контуров.

**Definition of done:** задача с пользовательским изменением не считается завершённой, пока хотя бы один применимый контур не обновлён или не проверен.
