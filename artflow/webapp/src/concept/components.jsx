...
export function Avatar({ user, size = "md" }) {
  ...
  return (
    <span className={`cxAvatar cxAvatar--${size}`} aria-label={name}>
      {photo && !failed
...
export function BottomNavigation({ screen, onNavigate }) {
  ...
  return (
    <nav className="cxBottomNav" aria-label="Основная навигация">
      {items.map(([id, icon, label]) => {
        if (id === "orb") {
          return (
            <button key={id} className="cxOrbButton" type="button" onClick={() => onNavigate("create")} aria-label="Создать">
              <span className="cxOrbButton__glow"/>
              <span className="cxOrbButton__core"><Icon name="sparkle" size={27}/></span>
            </button>
          );
        }
        return (
          <button key={id} type="button" className={screen === id ? "active" : ""} onClick={() => onNavigate(id)}>
...
export function Loading({ label = "Загружаем" }) {
  return (
    <div className="cxLoading" role="status">
      <span className="cxLoading__orb"><Icon name="sparkle" size={24}/></span>
      <b>{label}</b>
      <i/><i/><i/>
    </div>
  );
...
export function Notice({ notice, onClose }) {
  ...
  return (
    <div className={`cxNotice cxNotice--${notice.type || "info"}`} role={notice.type === "error" ? "alert" : "status"}>
...
export function MediaPlaceholder({ item, compact = false }) {
...
export function MediaViewer({ entry, onClose, onLike, onRemix, onShare }) {
  ...
  useEffect(() => {
    ...
    const keydown = (event) => {
      ...
      if (event.key === "ArrowRight") setIndex((value) => Math.min(previewCount - 1, value + 1));
    };
  ...
  function touchEnd(event) {
    ...
    if (end < start && safeIndex < previewCount - 1) setIndex((value) => value + 1);
    if (end > start && safeIndex > 0) setIndex((value) => value - 1);
  }

  return (
    <div className="cxViewer" role="dialog" aria-modal="true">
      <ProgressiveMedia item={item} index={safeIndex} sources={sources} className="cxViewer__media" controls autoPlay/>
      <div className="cxViewer__shade"/>

      <header className="cxViewer__header">
        <button type="button" onClick={onClose} aria-label="Закрыть"><Icon name="close" size={26}/></button>
        <div><Avatar user={{ username: item.author }} size="sm"/><span>@{item.author || "creator"}</span><i/></div>
        <button type="button" onClick={() => onShare(item)} aria-label="Поделиться"><Icon name="share" size={22}/></button>
      </header>

      <div className="cxViewer__gesture" onTouchStart={touchStart} onTouchEnd={touchEnd}>
        {safeIndex > 0 && <button type="button" className="prev" onClick={() => setIndex((value) => value - 1)}><Icon name="back"/></button>}
        {safeIndex < previewCount - 1 && <button type="button" className="next" onClick={() => setIndex((value) => value + 1)}><Icon name="chevron"/></button>}
      </div>

      <section className="cxViewer__content">
        <span className="cxViewer__tag"><Icon name="sparkle" size={14}/>{item.model || (item.gen_type === "video" ? "Видео" : "Фото")}</span>
        <h2>{prompt ? prompt.slice(0, 60) : "Новая работа"}</h2>
        <p>{item.gen_type === "video" ? "Видео" : "Кинематик"} <i/> {item.aspect_ratio || "9:16"}</p>
        <div className="cxViewer__stats">
          <span><Icon name="heart" size={16}/>{formatCompact(item.likes_count)}</span>
          <span><Icon name="eye" size={16}/>{formatCompact(item.views_count || item.remixes)}</span>
          <span><Icon name="reload" size={16}/>{formatCompact(item.remixes)}</span>
        </div>
        <div className="cxViewer__actions">
          <button type="button" onClick={() => onLike(item)}><Icon name="heart" size={19}/>Нравится</button>
          <button type="button" className="primary" onClick={() => onRemix(item)}><Icon name="sparkle" size={19}/>Повторить</button>
          <button type="button" onClick={() => onShare(item)}><Icon name="share" size={19}/>Поделиться</button>
        </div>
      </section>
    </div>
  );
...
export function DemoBanner() {
  ...
  return (
    <div className="cxDemoBanner">
      <Icon name="sparkle" size={17}/><span>Демо-режим. Генерации и платежи доступны внутри Telegram.</span>
    </div>
  );
}
